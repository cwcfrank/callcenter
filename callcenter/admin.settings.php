...
if($action == 'updatesettings'){
	...
	if(!is_writeable($filename)) {
		$errors = '請將系統配置文件config/settings.php設置為可寫, 即屬性設置為: 777';
	}

	if(isset($errors)){
		PrintErrors($errors, '系統設置錯誤');
...
if($action == 'displaysettings'){
	...
	<tbody>
	<tr>
	<td><B>前台默認語言</B><BR>當選擇 \'<b>自動</b>\' 時, 將根據訪客的瀏覽器語言自動選擇語言, 中文瀏覽器進入中文, 其它語言瀏覽器自動進入英文.</td>
	<td>';
	$Langs = GetLangs();
	...
	$Select->AddOption(8, '(GMT +8) 台灣, 北京, 香港, 新加坡');
	...
	<tr>
	<td><B>交互時間間隔</B><BR>客服端檢測服務器最新數據的時間間隔(秒).<BR>可設置為<span class=note>3-20</span>之間的整數, 數值越小交互速度越快, 但會增加服務器負擔.</td>
	<td>
	...
	<tr>
	<td><B>防惡意提交信息碼</B><BR>此碼有效防止機器人惡意留言, 提交信息等, 可時常更換, 但<span class=note>不能設置為空</span>.</td>
	<td>
	...
	<tr>
	<td><B>中文頁面標題</B><BR>當用戶使用中文瀏覽器時, 顯示在瀏覽器頂部的標題名稱.</td>
	<td>
	...
	<tr>
	<td><B>英文頁面標題</B><BR>當用戶使用非中文瀏覽器時, 顯示在瀏覽器頂部的標題名稱.</td>
	<td>
	...
	<tr>
	<td><B>自動刪除記錄</B><BR>客服或管理員登錄後是否自動刪除對話記錄. 自動刪除記錄有助於提高對話速度, 達到系統自我維護的目的.</td>
	<td>';
	$Select->Clear();
