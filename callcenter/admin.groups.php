...
if($action == 'updategroups'){
	...
	if(!storeCache()){ //更新小面板在线客服缓存文件
		$errortitle = '更新客戶緩存錯誤';
		$errors = '客服群組信息已保存到數據庫, 但更新在線客服緩存文件失敗, 前台客服小面板狀態無法更新! 請檢查cache/目錄是否存在或可寫?';
		$action = 'default';
	}else{
...
if($action == 'default'){
	...
	<td>中文说明:</td>
	<td>
	<textarea name="description" rows="4"  style="width:180px;">'.$usergroup['description'].'</textarea> <span class=note2>允许HTML, 如換行插入&lt;br&gt;</span>
	</td>
	</tr>
	<tr>
	<td>客服群組的英文名稱:</td>
	...
	<td>英文說明:</td>
	<td>
	<textarea name="descriptionen" rows="4"  style="width:180px;">'.$usergroup['descriptionen'].'</textarea> <span class=note2>允许HTML, 如換行插入&lt;br&gt;</span>
	</td>
	</tr>
	</tbody>
	</table>';
	...
	<input type="submit" name="deletegroups" onclick="return confirm(\'確定刪除所選客戶群組嗎?\r\n\r\n提示: 僅允許刪除沒有客服人員的群組!\');" value=" 刪除群組 " />
	</div>
	</form>';
